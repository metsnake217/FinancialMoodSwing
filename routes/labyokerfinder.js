...
MusicUpdateRules.prototype.updaterule = function(callback) {
	...
	var val = this.val;
	var sql = "Update rules set " + this.col +" = " + val/100 + " where name='"+this.rule+"'";
	console.log("sql: " + sql);
...
var ret = {index:"", desc:"", url:"", artist:"", title:"", mood: "", youtube:"", diff:0};
...
var vals = "", ids="";
	...
	for(var i in databuild){
    	...
    	vals += "('" +item.trackid + "','" + playlistname + "','" + item.tracktitle.replace("\'","") + "'," + item.mode_up + "," + item.mode_down + "," + item.mode_wayup + "," + item.mode_waydown + "," + item.prog_up + "," + item.prog_down + "," + item.prog_wayup + "," + item.prog_waydown + ",'" + item.trackurl + "','" + item.trackartist.replace("\'","") + "'),"
    	} else {
    		if(item.mode_up == 1){
